...
class TcpHolder {
 public:
  ...
  tcp_connection::pointer socket() { return socket_; };
 ...
 private:
  ...
  tcp_connection::pointer socket_;

  explicit TcpHolder(tcp_connection::pointer socket, InspectorSocket::DelegatePointer delegate);
  ~TcpHolder() = default;
 ...
 private:
  ...
  std::string HeaderValue(const std::string& header) const {
    ...
    for (const auto& header_value : headers_) {
      if (StringEqualNoCaseN(header_value.first.data(), header.data(),
                                   header.length())) {
        if (header_found)
          return "";
  ...
  bool IsAllowedHost(const std::string& host_with_port) const {
    ...
    return host.empty() || IsIPAddress(host)
           || StringEqualNoCase(host.data(), "localhost")
           || StringEqualNoCase(host.data(), "localhost6");
  }
...
TcpHolder::TcpHolder(tcp_connection::pointer socket, InspectorSocket::DelegatePointer delegate)
                     : delegate_(std::move(delegate)),
                       socket_(socket), handler_(nullptr) { }
...
// static
InspectorSocket::Pointer InspectorSocket::Accept(tcp_connection::pointer socket, DelegatePointer delegate) {
  auto tcp = TcpHolder::Accept(socket, std::move(delegate));
  if (tcp) {
